...
"The " << RESET << "\e[1mPASS\e[0m " << CYAN << "command is used to set a 'connection password'.The\n\
optional password can and MUST be set before any attempt to register the\n\
connection is made.  Currently this requires that user send a " << RESET << "\e[1mPASS\e[0m " << CYAN << "command\n\
before sending the " << RESET << "\e[1mNICK/USER\e[0m " << CYAN << "combination.\n" << RESET;
...
<hostname> <servername> <realname>\n\n"\
	<< CYAN << "This command is used at the beginning of a connection to specify\n\
the username, hostname, real name and initial user modes of the\n\
connecting client. <realname> may contain spaces, and thus must\n\
be prefixed with a colon.\n";
	std::cout << PURPLE << "\nExample:\n" << GREEN << "\e[1mUSER\e[0m " << 
	RESET << "guest tolmoon tolsun :Ronnie Reagan\n\n";
	return 0;
}
...
int Bot::quit() {
	...
	<< CYAN << "A client session is ended with a quit message.  The server must close\n\
the connection to a client which sends a " << RESET << "\e[1mQUIT\e[0m" 
<< CYAN << " message. If a \"Quit \n\
Message\" is given, this will be sent instead of the default message,\n\
the nickname.\n";
std::cout << PURPLE << "\nExample:\n" << GREEN << "\e[1mQUIT\e[0m " 
<< RESET << ":Gone to have lunch\n\n";
	return 0;
}
...
<< "\e[1mMODE\e[0m " << RESET << "#Finnish +v Wiz\n\n" <<
...
channel will be changed, if the channel modes permit this action.\n";
	std::cout << PURPLE << "\nExample:\n" << RESET << ":Wiz " << GREEN 
	<< "\e[1mTOPIC\e[0m " << RESET << "#test :New topic\n\n" 
